...
<main>
    <h1 style="text-align:center;margin-bottom:20px;font-family:'Times New Roman', Times, serif;">Rekap Keterangan Daftar Jurnal <br> Siswa SMKN 1 Cibinong</h1>
    ...
    <table style="width:100%;border-collapse:collapse;">
        ...
        @forelse($journals as $journal)
            <tr>
                ...
                <td style="border:1px solid #ddd;padding:8px;">{{ $journal->student->name ?? 'Nama Tidak Ditemukan' }}</td>
                <td style="border:1px solid #ddd;padding:8px;">{{ $journal->subject->name ?? 'Pelajaran Tidak Ditemukan' }}</td>
                <td style="border:1px solid #ddd;padding:8px;">{{ $journal->created_at->format('d-m-Y H:i') }}</td>
                <td style="border:1px solid #ddd;padding:8px;">{{ Str::limit($journal->description, 100) }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="5" style="text-align:center;padding:10px;">Tidak ada data jurnal.</td>
            </tr>
        @endforelse
        </tbody>
    </table>
